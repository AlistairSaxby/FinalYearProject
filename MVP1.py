...
def prediction():
    ...
    try:
        ...
        if type(userInput) == int or type(userInput) == float:
            calculatedoutput = linear.intercept_ + (linear.coef_ * userInput)
            print("The calculated HBV DNA Copy is =", calculatedoutput)
            ...
            if contribute == "y" or contribute == "Y" or contribute == "yes" or contribute == "Yes":
                print("Awesome!")
                adddata(userInput, calculatedoutput)
                train()
                menu()
            else:
                print("No problem!")
...
def groupprediction():
    ...
    while i != length:
        ...
        newcalculatedoutput = linear.intercept_ + (linear.coef_ * HBS)
